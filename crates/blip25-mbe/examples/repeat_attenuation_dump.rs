...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: repeat_attenuation_dump <in.ambe9>");
    ...
    println!(
        "Found {} Repeat/Mute events in {} frames.",
        events.len(),
        nf
    );
    println!(
        "{:>5} {:>5} {:>5} {:>10} {:>7} {:>8}",
        "frame", "ε₀", "ε_T", "disp", "peak", "rms"
    );
    for &ev in &events {
        ...
        for j in lo..hi {
            if shown.insert(j) {
                ...
                let marker = if !matches!(disp, FrameDisposition::Use) {
                    " <--"
                } else {
                    ""
                };
