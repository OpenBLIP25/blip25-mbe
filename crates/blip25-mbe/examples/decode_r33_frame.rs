...
use blip25_mbe::rate33::frame::{
    decode_frame, pack_no_fec, unpack_no_fec, DIBITS_PER_FRAME, INFO_WIDTHS,
};
...
fn main() {
    ...
    assert!(
        hex.len() == 18,
        "need 9 bytes (18 hex digits), got {} digits",
        hex.len()
    );
    ...
    match decode_to_params(&frame.info, &mut st) {
        Ok(Decoded::Voice(p)) => {
            ...
            println!(
                "kind: VOICE   f0 = {hz:.1} Hz   L = {}   omega_0 = {:.5}",
                p.harmonic_count(),
                p.omega_0()
            );
        }
    ...
    println!(
        "r34 → unpack roundtrip matches info: {}",
        back == frame.info
    );
}
