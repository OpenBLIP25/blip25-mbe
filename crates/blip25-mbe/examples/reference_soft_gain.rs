//! Soft-decision FEC gain test against the reference's own reference vectors.
//!
...
//!     (Chase-II soft Golay/Hamming, the reference's soft-decision path)
...
//!   * **33** — half-rate AMBE+2 / the reference Rate 33 (`rate33`, 72
...
//!
//! Usage:
//!   cargo run --release -p blip25-mbe --example reference_soft_gain -- [--rate 33] [VECTOR_DIR]

use blip25_mbe::reference_soft_decision::unpack_nibble_stream;

/// Default vector locations: the gitignored `reference-material/Vectors` symlink at the
/// workspace root, resolved from `CARGO_MANIFEST_DIR` so they do not depend on
/// the working directory.
const DEFAULT_DIR_FULL: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../reference-material/Vectors/tv-std/tv/p25"
);
const DEFAULT_DIR_R33: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../reference-material/Vectors/tv-rc/r33");
