//! Reference soft-decision decoder packet — the reference's handoff format.
//!
//! Per the reference application note on soft-decision error decoding for the
//! AMBE+2™ vocoder (2022). When a user wants to hand our
//! soft-decision channel bits off to **official reference silicon**
//! (USB-3000 / AMBE-3000 / AMBE-2000/2020) rather than this crate's
//! clean-room soft FEC, the reference's decoder accepts each encoded
...
//! [`crate::rate33::frame::decode_frame_soft`]) and the reference
//! SD format / packet.
//!
//! ## The 4-bit soft value (offset-binary)
//!
//! The reference's SD value is *not* sign-magnitude — it is a 16-level
...
//! soft-decision does nothing to the encoder packet." The reference does the
...
//!
//! The container is sized for the reference's maximum rate (9600 bps,
...
//!
//! ## Conventions confirmed against the reference vectors
//!
//! Confirmed against the reference's USB-3000 vectors
//! (`tv-std/tv/p25/clean_e1_sd.bit` against the hard-bit `clean.bit`):
//!
//! * **SD encoding** — offset-binary with MSB = hard bit. The SD-nibble
//!   hard decisions track the hard reference to within the vector's
//!   injected 1% BER; the three other nibble/bit-order conventions score
//!   at chance, so this one is not a coin flip.
//! * **Nibble order** — `SD0` is the high nibble of each byte
//!   ([`pack_nibble_stream`]).
//! * **Channel-bit ordering** — sequential and identical between the reference's
//!   soft and hard files, and the `imbe7200` bit order matches these same
//!   `tv/p25` vectors, so a 144-bit soft frame maps straight to
//!   `SD0..SD143`.
//! * **Rate-control words** — taken from the reference command file
//!   `cmpp25.txt`; see [`P25_FULLRATE_FEC`] / [`P25_FULLRATE_NOFEC`].
//!
//! Caveat: the AMBE-2000/2020 **host packet**'s five `rate_info` words
//! ([`SdPacketHeader::rate_info`]) are a *different* framing from the
//! 6-word `-r` rate-control vector, are not exercised by the available
//! USB-3000 vectors, and stay caller-supplied. The raw
//! [`pack_nibble_stream`] format (what the vectors use) carries no rate
//! words at all.

/// Packet header word — always `0x13EC` (Table 1, word 0).
pub(crate) const SD_HEADER: u16 = 0x13EC;
...
pub(crate) const SD_OVERHEAD_WORDS: usize = 12;

/// Number of 16-bit SD data words (words 12..60), four nibbles each.
pub(crate) const SD_DATA_WORDS: usize = 48;

/// Maximum soft-decision slots in one packet (`SD0..SD191`), = the
/// reference's highest-rate channel-bit count (9600 bps).
pub(crate) const SD_SLOTS: usize = 192;
...
/// meaning `1`, magnitude = confidence in `0..=127`) into a reference 4-bit
...
/// Inverse of [`llr_to_sd_nibble`]: map a reference 4-bit SD value back to a
...
/// control words are reference-/rate-specific and caller-supplied** — see the
/// module-level caveat. `Default` zeroes everything except via
/// [`SdPacketHeader::new`].
...
pub struct SdPacketHeader {
    ...
    /// Words 2..7 — Rate info 0..4. Reference-/rate-specific; not derivable
    /// from the app note. Source from the reference's rate-control spec.
    pub rate_info: [u16; 5],
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::TooManyChannelBits(n) => {
                write!(
                    f,
                    "{n} channel bits exceeds the {SD_SLOTS}-slot reference SD packet"
                )
            }
...
/// Pack soft channel bits (in **reference encoded-packet order** — see the
/// module-level conventions note) into a reference AMBE-2000/2020
...
/// Unpack a reference soft-decision packet into its header and the full
...
// ---------------------------------------------------------------------------
//
// the reference's USB-3000 reference vectors store soft-decision channel bits as a
...
// out-of-band via the reference's `-r` rate-control vector). This is the
// directly-useful handoff format for piping soft bits to `reference_client`
// or comparing against the `*_sd.bit` test vectors.
//
// Only this exact convention (hard byte MSB-first, SD0 = high nibble,
// SD-value MSB = hard bit) reproduces `tv-std/tv/p25/clean_e1_sd.bit`
// against the hard `clean.bit`; the three other nibble/bit-order
// conventions score at chance. Do not flip any of the three.
...
// ---------------------------------------------------------------------------
// Verified the reference rate-control vectors (the reference `-r` flag)
// ---------------------------------------------------------------------------
//
// Source: the reference USB-3000 reference command file `tv-std/tv/cmpp25.txt`
// (`-c P25 -r <six u16 words>`). These are the 6-word rate-control vector
// passed to the reference out-of-band, NOT the AMBE-2000/2020 host packet's
// 5-word `rate_info` field (a different framing). Use these when driving a
// USB-3000 / AMBE-3000-series reference for P25 full-rate.

/// P25 full-rate (7200 bps IMBE) **with FEC** — the reference `-r` rate control.
pub const P25_FULLRATE_FEC: [u16; 6] = [0x0558, 0x086B, 0x1030, 0x0000, 0x0000, 0x0190];

/// P25 full-rate (7200 bps IMBE) **without FEC** — the reference `-r` rate control.
pub const P25_FULLRATE_NOFEC: [u16; 6] = [0x0558, 0x086B, 0x0000, 0x0000, 0x0000, 0x0158];
...
mod tests {
    ...
    fn nibble_stream_sd0_is_high_nibble() {
        // the reference convention: SD0 in the high nibble of byte 0.
