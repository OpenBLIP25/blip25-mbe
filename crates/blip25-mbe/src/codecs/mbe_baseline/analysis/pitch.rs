...
/// Experimental: Viterbi pitch tracker over the Eq. 11 half-sample grid.
///
/// Alternative to §0.3.4–§0.3.6's look-back / look-ahead / Eq. 21-23
/// decision. Keeps the spec's `E(P)` as the per-frame emission cost
/// (so each candidate's fit to Eq. 5 is unchanged) but layers a
/// continuity transition cost on top: penalize octave-scale log-pitch
/// jumps between adjacent frames. Finds the maximum-a-posteriori
/// pitch trajectory across a sliding window of frames.
///
/// Rationale (2026-04-24 baseline data): our §0.3 tracker finds the
/// spec-optimal pitch on most frames but has ~12% of voice frames at
/// |Δ| > 200¢ vs the chip — onset transitions, sticky-lock on
/// super-multiples, low-F0 near grid edges. Those failures are
/// well-studied in the pitch-tracking literature; modern trackers
/// (pYIN, probabilistic Viterbi over candidate grids) address them
/// cleanly. This is a minimum-intervention variant: same emission
/// model, just add a pairwise continuity prior.
///
/// Feature-flagged; off by default. Gate via
/// [`super::AnalysisState::set_viterbi_pitch`].
#[derive(Clone, Debug)]
pub struct ViterbiPitchTracker {
    /// Previous frame's per-candidate accumulated log-cost (running
    /// Viterbi state). `None` on cold start.
    prev_cost: Option<Vec<f64>>,
    /// Previous frame's selected pitch. Used as a fallback on silent
    /// frames where all costs collapse.
    prev_pitch: f64,
}

/// Transition cost coefficient on squared log-pitch delta between
/// adjacent frames. A value of 4.0 makes a 1-octave (±12 semitones)
/// jump cost `4 · ln(2)² ≈ 1.92` emission units — roughly the same
/// order as an E(P) difference from "good fit" (~0.1) to "poor fit"
/// (~0.8), so the Viterbi will prefer a 0.5-unit-worse E(P) over a
/// one-octave jump. Calibrated to discourage octave lock-ups while
/// still tracking genuine pitch changes.
const VITERBI_TRANSITION_WEIGHT: f64 = 4.0;

/// Floor on E(P) to prevent log(0). `E(P) = 0` only on perfectly
/// periodic synthetic signals; real speech always has E > 0.
const VITERBI_EMISSION_FLOOR: f64 = 1e-6;

impl ViterbiPitchTracker {
    /// Cold-start tracker (no prior frame history).
    pub const fn cold_start() -> Self {
        Self { prev_cost: None, prev_pitch: PITCH_COLD_START }
    }

    /// Advance one frame and return the maximum-a-posteriori pitch for
    /// the current frame. `search` gives access to `E(P)` on the
    /// half-sample grid.
    ///
    /// Emission cost on frame `t` at candidate `P_i` is
    /// `max(E(P_i), ε)`; transition cost from prev-frame `P_{i'}` to
    /// current `P_i` is `w · (ln(P_i / P_{i'}))²`. Viterbi minimizes
    /// the sum over the trajectory.
    ///
    /// Returns `(P̂, E(P̂))` — the chosen pitch and its emission cost
    /// at the current frame. `E(P̂)` is the raw Eq. 5 value (NOT the
    /// Viterbi accumulated cost) so downstream consumers like
    /// `e_p_hat_i` stay comparable to the spec tracker.
    pub fn step(&mut self, search: &PitchSearch) -> (f64, f64) {
        let n = PITCH_GRID_LEN;
        // Emission costs on the Eq. 11 grid.
        let mut emit = Vec::with_capacity(n);
        for i in 0..n {
            let p = PITCH_GRID_MIN + (i as f64) * PITCH_GRID_STEP;
            let e = search.e_of_p(p).max(VITERBI_EMISSION_FLOOR);
            emit.push(e);
        }
        let mut cur_cost = vec![0.0f64; n];
        match &self.prev_cost {
            None => {
                // Cold start: accumulated cost = emission alone.
                cur_cost.copy_from_slice(&emit);
            }
            Some(prev) => {
                // For each current candidate, find the min over
                // previous candidates of (prev_cost + transition).
                for i in 0..n {
                    let p_i = PITCH_GRID_MIN + (i as f64) * PITCH_GRID_STEP;
                    let ln_p_i = p_i.ln();
                    let mut best = f64::INFINITY;
                    for j in 0..n {
                        let p_j = PITCH_GRID_MIN + (j as f64) * PITCH_GRID_STEP;
                        let d = ln_p_i - p_j.ln();
                        let trans = VITERBI_TRANSITION_WEIGHT * d * d;
                        let total = prev[j] + trans;
                        if total < best {
                            best = total;
                        }
                    }
                    cur_cost[i] = best + emit[i];
                }
            }
        }
        // Argmin of the current accumulated cost → MAP pitch for
        // frame t (last-step assignment; no backtracking for an
        // online tracker, which matches §0.3's online semantics).
        let mut best_idx = 0usize;
        let mut best_cost = cur_cost[0];
        for i in 1..n {
            if cur_cost[i] < best_cost {
                best_cost = cur_cost[i];
                best_idx = i;
            }
        }
        let p_hat = PITCH_GRID_MIN + (best_idx as f64) * PITCH_GRID_STEP;
        let e_p_hat = emit[best_idx].min(1.0); // clamp floored emission back into spec range
        self.prev_cost = Some(cur_cost);
        self.prev_pitch = p_hat;
        (p_hat, e_p_hat)
    }

    /// Reset the tracker to cold-start state (e.g., after a silence
    /// run where the trajectory would otherwise drag across the gap).
    pub fn reset(&mut self) {
        self.prev_cost = None;
        self.prev_pitch = PITCH_COLD_START;
    }
}

impl Default for ViterbiPitchTracker {
    fn default() -> Self {
        Self::cold_start()
    }
}
...
mod tests {
    ...
    /// Cold-start Viterbi tracker reduces to per-frame argmin of
    /// `E(P)` on the Eq. 11 grid (no prior history → transition cost
    /// is zero, so only emission matters on frame 1).
    #[test]
    fn viterbi_cold_start_matches_grid_argmin() {
        let s = periodic_input(50.0);
        let search = PitchSearch::new(&s);
        let mut t = ViterbiPitchTracker::cold_start();
        let (p_hat, _) = t.step(&search);
        let (p_brute, _) = search.argmin_in_range(PITCH_GRID_MIN, PITCH_GRID_MAX);
        assert_eq!(
            p_hat, p_brute,
            "Viterbi cold-start pick = {p_hat}, brute grid-argmin = {p_brute}"
        );
    }

    /// Viterbi transition cost penalizes octave jumps: two adjacent
    /// frames with the same emission landscape should stabilize at
    /// the same pitch on the second frame, not jump to an equal-E
    /// sub-multiple.
    #[test]
    fn viterbi_stabilizes_on_repeated_emissions() {
        let s = periodic_input(50.0);
        let search = PitchSearch::new(&s);
        let mut t = ViterbiPitchTracker::cold_start();
        let (p1, _) = t.step(&search);
        let (p2, _) = t.step(&search);
        assert_eq!(
            p1, p2,
            "repeated emission should give same pitch: p1={p1}, p2={p2}"
        );
    }

    /// Viterbi returns pitch on the Eq. 11 half-sample grid.
    #[test]
    fn viterbi_returns_grid_value() {
        let s = periodic_input(50.0);
        let search = PitchSearch::new(&s);
        let mut t = ViterbiPitchTracker::cold_start();
        let (p, _) = t.step(&search);
        let doubled = (p * 2.0).round();
        assert!(
            (doubled / 2.0 - p).abs() < 1e-9,
            "Viterbi pick {p} not on half-sample grid"
        );
        assert!((PITCH_GRID_MIN..=PITCH_GRID_MAX).contains(&p));
    }
