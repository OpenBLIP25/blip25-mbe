...
fn unpack_msb(bytes: &[u8]) -> [u8; 72] {
    let mut out = [0u8; 72]; let mut bit = 0usize;
    for slot in &mut out {
        let mut d = 0u8;
        for _ in 0..2 { let b = (bytes[bit/8] >> (7-(bit%8))) & 1; d = (d<<1)|b; bit+=1; }
        *slot = d;
    }
    out
}
...
fn write_wav(path: &str, samples: &[i16]) {
    use std::io::Write;
    let sr = 8000u32; let n = samples.len() as u32;
    let mut f = fs::File::create(path).unwrap();
    let data_size = n*2; let chunk_size = 36 + data_size;
    f.write_all(b"RIFF").unwrap(); f.write_all(&chunk_size.to_le_bytes()).unwrap();
...
fn main() {
    ...
    for f in 0..n {
        ...
        let params = match dequantize(&imbe.info, &mut dec) {
            Ok(p) => p,
            Err(_) => { pcm.extend(std::iter::repeat(0i16).take(160)); continue; }
        };
